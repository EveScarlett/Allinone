...
<div class="ubicaciones-form p-3">
    ...
    <div class="row mt-3">
        <div class="col-lg-12">
            <?= $form->field($model, 'nivelorganizacional2')->textInput(['maxlength' => true,'onkeyup' => 'converToMayus(this);'])->label($label); ?>
        </div>
    </div>
    ...
    <div class="container p-3 shadow rounded-3 mt-5">
        ...
        if($model){
            ...
            if($nivel == $cantidad_niveles){
                $trabajadores_activos = Trabajadores::find()->where(['id_empresa'=>$empresa->id])->andWhere(['id_nivel1'=>$nivel_1->id])->andWhere(['id_nivel2'=>$model->id])->andWhere(['in','status',[1,3]])->all();
            }
            ...
            if($nivel == $cantidad_niveles){
                ...
                $retpuestos = Puestostrabajo::find()->where(['id_empresa'=>$empresa->id])->andWhere(['id_nivel2'=>$model->id])->andWhere(['status'=>1])->all();
            }
            ...
            foreach($retareas as $key=>$data){
                ...
                $trabajadores_area = Trabajadores::find()->where(['id_empresa'=>$empresa->id])->andWhere(['id_nivel1'=>$nivel_1->id])->andWhere(['id_nivel2'=>$model->id])->andWhere(['id_area'=>$data->id])->andWhere(['in','status',[1,3]])->all();
            ...
            foreach($retpuestos as $key=>$data){
                ...
                $trabajadores_puesto = Trabajadores::find()->where(['id_empresa'=>$empresa->id])->andWhere(['id_nivel1'=>$nivel_1->id])->andWhere(['id_nivel2'=>$model->id])->andWhere(['id_puesto'=>$data->id])->andWhere(['in','status',[1,3]])->all();
            ...
            foreach($retkpis as $key=>$data){
                ...
                if($data->kpi == 'D'){
                    ...
                    if($data->kpi == 'A'){//ACCIDENTES
                        ...
                        $accidentes_kpi = Consultas::find()->where(['id_empresa'=>$empresa->id])->andWhere(['id_nivel1'=>$nivel_1->id])->andWhere(['id_nivel2'=>$nivel_2->id])->andWhere(['in','id_trabajador',$id_trabajadores])->andWhere(['tipo'=>1])->andWhere(['IS', 'status_baja', new \yii\db\Expression('NULL')])->all();
                        ...
                        $hoy = date('Y-m-d');
                        $incapacidades_kpi = [];
                        $incapacidades_kpi = Consultas::find()->where(['id_empresa'=>$empresa->id])->andWhere(['id_nivel1'=>$nivel_1->id])->andWhere(['id_nivel2'=>$nivel_2->id])->andWhere(['in','id_trabajador',$id_trabajadores])->andWhere(['tipo'=>4])->andWhere(['>=','incapacidad_fechafin',$hoy])->andWhere(['IS', 'status_baja', new \yii\db\Expression('NULL')])->all();
                        $qty = count($incapacidades_kpi);

                    } else if($data->kpi == 'E'){//POES
                        $anio_before = date('Y-m-d', strtotime('-1 years'));
                        $poes_kpi = [];
                        $poes_kpi = Poes::find()->where(['id_empresa'=>$empresa->id])->andWhere(['id_nivel1'=>$nivel_1->id])->andWhere(['id_nivel2'=>$nivel_2->id])->andWhere(['in','id_trabajador',$id_trabajadores])->andWhere(['>=','create_date',$anio_before])->andWhere(['<>', 'status', 2])->all();
                        $qty = count($poes_kpi);
                    }
                }
                $content_kpis .= '
