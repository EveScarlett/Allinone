...
<div class="poes-index">
    ...
    <?php
        ...
        if(isset($searchModel->id_empresa) && $searchModel->id_empresa != null && $searchModel->id_empresa != ''){
            $querypoes->andFilterWhere(['=', 'id_empresa', $searchModel->id_empresa]);
        }
    
        if(isset($searchModel->id_trabajador) && $searchModel->id_trabajador != null && $searchModel->id_trabajador != ''){
            $querypoes->andFilterWhere(['like', 'Concat(nombre," ", apellidos)', $searchModel->id_trabajador]);
        }
    
        if(isset($searchModel->condicion) && $searchModel->condicion != null && $searchModel->condicion != ''){
            $ids_trabajadores = [];
            
            if($searchModel->condicion == 5){
                $items = Trabajadores::find()->where(['status'=>5])->all();
            }else{
                $items = Trabajadores::find()->where(['status'=>$searchModel->condicion])->all();
            }
            ...
            if(count($ids_trabajadores)>0){
                $querypoes->andFilterWhere(['in', 'id_trabajador', $ids_trabajadores]);
            }else{
                $querypoes->andFilterWhere(['id'=>'0']);
            } 
        }
    
        if(isset($searchModel->sexo) && $searchModel->sexo != null && $searchModel->sexo != ''){
            $querypoes->andFilterWhere(['sexo' => $searchModel->sexo]);
        }
    
        if(isset($searchModel->anio) && $searchModel->anio != null && $searchModel->anio != ''){
            $querypoes->andFilterWhere(['anio' => $searchModel->anio]);
        }
    
        if(isset($searchModel->create_date) && $searchModel->create_date != null && $searchModel->create_date != ''){
            
            $fechas = explode( ' - ', $searchModel->create_date);
            $querypoes->andFilterWhere(['between','create_date', $fechas[0], $fechas[1]]);
            //dd($querypoes);
        }
    
        if(isset($searchModel->id_puesto) && $searchModel->id_puesto != null && $searchModel->id_puesto != ''){
            $ids_puestos = [];
    
            $items = Puestostrabajo::find()->where(['like','nombre',$searchModel->id_puesto])->all();
            ...
            if(count($ids_puestos)>0){
                $querypoes->andFilterWhere(['in', 'id_puesto', $ids_puestos]);
            }else{
                $querypoes->andFilterWhere(['id'=>'0']);
            }  
        }
    
        if(isset($searchModel->id_area) && $searchModel->id_area != null && $searchModel->id_area != ''){
            $ids_areas = [];
    
            $items = Areas::find()->where(['like','area',$searchModel->id_area])->all();
            ...
            if(count($ids_areas)>0){
                $querypoes->andFilterWhere(['in', 'id_area', $ids_areas]);
            }else{
                $querypoes->andFilterWhere(['id'=>'0']);
            }  
        }
    
        if((isset($searchModel->estudios) && $searchModel->estudios != null && $searchModel->estudios != '') || (isset($searchModel->diagnosticos) && $searchModel->diagnosticos != null && $searchModel->diagnosticos != '') || (isset($searchModel->evolucion) && $searchModel->evolucion != null && $searchModel->evolucion != '')){
            $ids_poes = [];
    
            $estudios = Poeestudio::find();
    
            if(isset($searchModel->estudios) && $searchModel->estudios != null && $searchModel->estudios != ''){
                $estudios->andFilterWhere(['in','id_estudio',$searchModel->estudios]);
            }
            if(isset($searchModel->diagnosticos) && $searchModel->diagnosticos != null && $searchModel->diagnosticos != ''){
                $estudios->andFilterWhere(['condicion'=>$searchModel->diagnosticos]);
            }
            if(isset($searchModel->evolucion) && $searchModel->evolucion != null && $searchModel->evolucion != ''){
                $estudios->andFilterWhere(['evolucion'=>$searchModel->evolucion]);
            }
            ...
            if(count($ids_poes)>0){
                $querypoes->andFilterWhere(['in', 'id', $ids_poes]);
            }else{
                $querypoes->andFilterWhere(['id'=>'0']);
            }  
        }
        ...
        $ids_poes = [];
        foreach($poes as $item){
            if(!in_array($item->id, $ids_poes)){
                array_push($ids_poes, $item->id);
            }
        }
        ...
        $ids_estudios = [];
        foreach($estudiospoes as $key=>$item){
            if(!in_array($item->id_estudio, $ids_estudios)){
                array_push($ids_estudios, $item->id_estudio);
            }
        }

        $estudiospoes = Servicios::find()->where(['in','id',$ids_estudios])->all();

        $columnas_estudios = [];

        foreach($estudiospoes as $key=>$estudio){ 
            $columna = [
                'attribute' =>'atributo'.$key.'_'.$estudio->id,
                'label'=>'CONDICION-'.$estudio->nombre.'',
                'value'=>function($model) use($estudio){
                    $ret = 'NO APLICA';
                    
                    $diagnosticos = ['100'=>'FALTA','0'=>'PENDIENTE','1'=>'BIEN','2'=>'REGULAR','3'=>'MAL'];
                    $det = Poeestudio::find()->where(['id_poe'=>$model->id])->andWhere(['id_estudio'=>$estudio->id])->one();
                    
                    if(isset($det)){
                        
                        if(isset($det->condicion) && $det->condicion != null && $det->condicion != ''){
                            $ret =  $diagnosticos[$det->condicion];
                        } else if($det->condicion == 0){
                            $ret = 'PENDIENTE';
                        }
                    }
                    return $ret;
                },
                /* 'exportMenuStyle' => function($model, $key, $index, $column) use($estudio){ 
                    $colors = [
                        '0'=>'FFBBACF2','1'=>'FF01D28E','2'=>'FFF1C93B','3'=>'FFFF597B'
                    ];
            ...
            $columna2 = [
                ...
                'value'=>function($model) use($estudio){
                    ...
                    if(isset($det)){
                        ...
                        if($det->id_estudio == 1 && $det->id_hc != null){
                            $ret =  'CON ARCHIVO';
                        } else {
                            if((isset($det->evidencia) && $det->evidencia != null && $det->evidencia != '') || (isset($det->evidencia2) && $det->evidencia2 != null && $det->evidencia2 != '') || (isset($det->evidencia3) && $det->evidencia3 != null && $det->evidencia3 != '')){
                                $ret =  'CON ARCHIVO';
                            }
                        }
            ...
            array_push($columnas_estudios, $columna);
            array_push($columnas_estudios, $columna2);
        }

        $columnas = array_merge($columnas, $columnas_estudios);
        //dd($columnas);
    ...
    <?php 
       $fullExportMenu =  '';
      if(Yii::$app->user->can('poes_exportar')){
        ...
        'columns' => [
            ...
            [
                ...
                'value'=>function($model){
                    ...
                    if($model->trabajador){
                        $ret = $numero.$model->trabajador->nombre.' '.$model->trabajador->apellidos;
                    }
                    return $ret;
                  },
            ],
